...
void hourInc(void){
	...
	if (interruptTime - lastInterruptTime>200){
		...
		//Fetch RTC Time
		int temp = wiringPiI2CReadReg8(RTC, HOUR_REGISTER);
		temp = hexCompensation(temp);
		...
		//Increase hours by 1, ensuring not to overflow
                ...
                hours = wiringPiI2CWriteReg8(RTC,HOUR_REGISTER,decCompensation(temp));
	}
...
void minInc(void){
	...
	if (interruptTime - lastInterruptTime>200){
		...
		//Fetch RTC Time
		int temp = wiringPiI2CReadReg8(RTC, MIN_REGISTER);
		temp = hexCompensation(temp);
		...
		mins = wiringPiI2CWriteReg8(RTC,MIN_REGISTER,decCompensation(temp));
	}
	lastInterruptTime = interruptTime;
}
